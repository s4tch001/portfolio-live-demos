...
Phase 2.2 adds a private, RLS-enabled reset-control schema and a service-to-service Edge coordinator. All five applications are registered but disabled and not resettable until a later app migration installs and enables its reviewed `reset_demo_data(date)` handler. No migration or function has been deployed remotely yet.
